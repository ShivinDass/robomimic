...
class SequenceDatasetMultiFile(torch.utils.data.Dataset):
    def __init__(
        ...
        assert isinstance(hdf5_path, list), "hdf5_path must be a list of paths"

        self.hdf5_paths = [os.path.expanduser(hdf5_pth) for hdf5_pth in hdf5_path]
        self.hdf5_use_swmr = hdf5_use_swmr
        self.hdf5_normalize_obs = hdf5_normalize_obs
        ...
        self.obs_normalization_stats = None
        if self.hdf5_normalize_obs:
            raise NotImplementedError("normalization not implemented for multi-file datasets")
            self.obs_normalization_stats = self.normalize_obs()
    ...
    def load_dataset_in_memory(self, demo_list, hdf5_files, obs_keys, dataset_keys, load_next_obs):
        ...
        for file_idx in range(len(demo_list)):
            print("SequenceDataset: loading dataset into memory...")
            all_data.append({})
            for ep in LogUtils.custom_tqdm(demo_list[file_idx]):
